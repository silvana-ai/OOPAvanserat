...
namespace AdoDotnetDAL
{
    public abstract class TemplateADO<AnyType> : AbstractDal<AnyType>
    {
        ...
        public void Execute(AnyType obj) // här fixar Sequence Insert
        {
            Open();
            ExecuteCommand(obj);
            Close();
        }
        public List<AnyType> Execute() // här när man fixar Sequence select
        {
            Open();
        ...
        protected override void ExecuteCommand(CustomerBase obj)
        {
            if (obj.Id == 0)
            {
                objCommand.CommandText = "insert into tblCustomer(" +
                                                "CustomerName," +
                                                "BillAmount,BillDate," +
                                                "PhoneNumber,Email,CustomerType)" +
                                                "values('" + obj.CustomerName + "'," +
                                                obj.BillAmount + ",'" +
                                                obj.BillDate + "','" +
                                                obj.PhoneNumber + "','" +
                                                obj.Email + "','" + obj.CustomerType + "')";
